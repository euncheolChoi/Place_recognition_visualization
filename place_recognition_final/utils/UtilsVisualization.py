...
class VizTrajectory:
    def __init__(self, node, pose):
        self.src = node[:, 0].astype(int)   # Node <- Loop
    ...
    def viz3D(self):
        ...
        print("TP_src", self.TP_src)
        print("FP_src", self.FP_src)
        
        print("TP_dst", self.TP_dst)
        print("FP_dst", self.FP_dst)